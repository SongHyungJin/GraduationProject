...
form_filter = uic.loadUiType(BASE_DIR +  '/' +"Second.ui")[0]


class ThreadFilter(QThread):
    user_signal = pyqtSignal(str, str)

    def __init__(self, parent, curFolder, lstRa, idxRa, lstRs, idxRs):
        super().__init__(parent)
        self.parent = parent
        self.currentFolder = curFolder
        self.listRate = lstRa
        self.idxRate = idxRa
        self.listResize = lstRs
        self.idxResize = idxRs
    ...
    def load_File(self, folder):
        ...
        for filename in filenames:  # 아래 확장자에 해당하는 파일만 열기-PIL로 바꾸고 변동 가능성**
            if filename.endswith(".bmp") or filename.endswith(".jpg") or filename.endswith(
                    ".jpeg") or filename.endswith(".png"):  # or filename.endswith(".webp"):
    ...
    # 완료 알림
    def fin_filter(self):
        self.user_signal.emit('알림', '작업 완료')
...
class FilterWindow(QDialog, QWidget, form_filter):
    ...
    def initUI(self):
        ...
        # 중복 처리 작업 실행
        self.BtnRun.clicked.connect(self.filter_start)
    ...
    def filter_start(self):
        ...
        Thread1 = ThreadFilter(self, self.currentFolder, self.listRate, self.idxRate, self.listResize, self.idxResize)
